...
use super::{
    create_descriptor_set_layout, BufferHandle, BufferPool, BufferSlice, DescriptorSetLayout,
    DescriptorSetLayoutDesc, DropList, Error, GpuDescriptor, GpuDescriptorAllocator, ImageHandle,
    ImagePool, SamplerDesc,
};
...
impl<'a> DescriptorSetSlot<'a> {
    fn get_slot<T: Copy>(
        ...
        let slot = match self {
            DescriptorSetSlot::Name(name) => layout.find_slot_by_name(name)?,
...
impl DescriptorSetManager {
    ...
    pub fn destroy_descriptor_set(&mut self, drop_list: &mut DropList, handle: DescriptorSetHandle) {
